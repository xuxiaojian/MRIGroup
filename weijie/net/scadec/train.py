...
class Trainer_bn(object):
    ...
    def _initialize(self, training_iters, output_path, restore, prediction_path):
        ...
        shutil.copytree('/export/project/gan.weijie/MRIGroup/weijie/', output_path + '/code/')

        return init

    def train(self, data_provider, output_path, valid_provider, valid_size, data_imgs, valid_imgs,
              training_iters=100, epochs=1000, dropout=0.75, display_step=20,
              save_epoch=50, restore=False, write_graph=False, prediction_path='validation', config_info='config'):
        ...
        with tf.Session(config=config) as sess:
            ...
            summary_writer = recorder.TBXWriter(output_path, config_info)
            logging.info("Start optimization")

            # select validation dataset
            valid_x, valid_y = valid_provider(valid_size, fix=True)
            valid_imgs_x, valid_imgs_y = valid_imgs
            data_imgs_x, data_imgs_y = data_imgs

            summary_writer.imgs_train_init(data_imgs_x, data_imgs_y)
            summary_writer.imgs_valid_init(valid_imgs_x, valid_imgs_y)
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch*training_iters), ((epoch+1)*training_iters)):
                    ...
                    summary_writer.train_batch(loss=loss, psnr=avg_psnr, ssim=avg_ssim, step=step)
                ...
                total_psnr /= training_iters
                total_ssim /= training_iters

                summary_writer.train_epoch(total_loss, total_psnr, total_ssim, step)

                loss, avg_psnr, avg_ssim = sess.run([self.net.loss,
                                                        self.net.avg_psnr,
                                                        self.net.avg_ssim],
                ...
                summary_writer.valid_epoch(loss, avg_psnr, avg_ssim, step)

                prediction = sess.run(self.net.recons,feed_dict={self.net.x: data_imgs_x,
                                                    self.net.keep_prob: 1.,
                                                    self.net.phase: False})

                summary_writer.imgs_train_epoch(prediction, step)

                prediction = sess.run(self.net.recons, feed_dict={self.net.x: valid_imgs_x,
                                                     self.net.keep_prob: 1.,
                                                     self.net.phase: False})

                summary_writer.imgs_valid_epoch(prediction, step)
