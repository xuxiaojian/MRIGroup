...
from net.scadec.layers import *
from net.scadec.nets import *
...
class Unet_bn(object):
    ...
    def __init__(self, img_channels=3, truth_channels=3, cost="mean_squared_error", cost_kwargs={}, **kwargs):
        ...
        self.avg_ssim = tf.reduce_mean(tf.image.ssim(self.recons, self.y, 1))
    ...
    def _get_cost(self, cost_name, cost_kwargs):
        ...
        if cost_name == "mean_squared_error":
            ...
            if cost_name == 'total_variation':
                loss = tf.losses.mean_squared_error(flat_recons, flat_truths)

                tv_recons_dif1 = self.recons[:, 1:, :-1, :] - self.recons[:, :-1, :-1, :]
                tv_recons_dif2 = self.recons[:, :-1, 1:, :] - self.recons[:, :-1, :-1, :]
                tv_recons = tf.abs(tv_recons_dif1) + tf.abs(tv_recons_dif2)

                tv_y_dif1 = self.y[:, 1:, :-1, :] - self.y[:, :-1, :-1, :]
                tv_y_dif2 = self.y[:, :-1, 1:, :] - self.y[:, :-1, :-1, :]
                tv_y = tf.abs(tv_y_dif1) + tf.abs(tv_y_dif2)

                loss += 0.01*tf.losses.mean_squared_error(tv_recons, tv_y)
