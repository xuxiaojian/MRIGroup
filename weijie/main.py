...
num_gpu = np.fromstring(config['GLOBAL']['index_gpu'], dtype=int, sep=',').shape[0]
print('Using [', num_gpu, '] GPUs')

dataset_path = config['GLOBAL']['dataset_path']

if len(sys.argv) == 3 and str(sys.argv[1]) == 'scadec':

    if str(sys.argv[2]) == 'train':

        x_train, y_train, x_val, y_val = dataLoader.train_unet(dataset_path)
        imgs_train = scadec.SimpleDataProvider(x_train, y_train)
        imgs_val = scadec.SimpleDataProvider(x_val, y_val)

        net = scadec.TFTrainer(scadec.TFNetwork(), batch_size=6)
        net.train(data_provider=imgs_train, output_path='/home/xiaojianxu/gan/experiment/dec31/unet_ver3/',
                  valid_provider=imgs_val, valid_size=4, training_iters=150, epochs=1000,
                  validation_path='/home/xiaojianxu/gan/experiment/dec31/unet_ver3/val/')
